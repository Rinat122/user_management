...
class TestUserManagementSystem(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.user = User(self.user_id, "Rinat", "Kubanychbekov", datetime(2005, 3, 21))
        self.user.email = UserUtil.generate_email("Rinat", "Kubanychbekov", "gmail.com")
        self.user.password = UserUtil.generate_password()

    def test_user_creation(self):
        self.assertEqual(self.user.name, "Rinat")
        self.assertEqual(self.user.surname, "Kubanychbekov")
    ...
    def test_user_update(self):
        UserService.add_user(self.user)
        updated_user = User(self.user_id, "Rinat", "Kubanychbekov", datetime(2005, 3, 21))
        UserService.update_user(self.user_id, updated_user)
        self.assertEqual(UserService.find_user(self.user_id).name, "Rinat")
